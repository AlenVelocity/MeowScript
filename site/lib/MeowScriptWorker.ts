...
importScripts('/meowscript.js')

export class MeowScriptWorker {
  module: any

  async initWasm() {
    // @ts-ignore
    // eslint-disable-next-line no-undef
    this.module = await MeowScript({
      mainScriptUrlOrBlob: '/meowscript.js',
      locateFile: (path: string) => {
        if (path === 'meowscript.wasm') {
          return `/${path}`
        }
      },
      print: (t: string) => {
...
Comlink.expose(MeowScriptWorker)
